    ...
    <>
      <div className="h-full bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden flex flex-col">
        <div className="px-6 py-4 bg-red-50/50 border-b border-gray-100 flex flex-row justify-between items-center gap-4 shrink-0">
          ...
          <div className="flex gap-3 text-[10px] font-black uppercase tracking-widest items-center shrink-0">
            ...
            {isEditing ? (
              <button onClick={handleSave} disabled={isPending} className="flex items-center gap-1 text-zinc-400 rounded-md hover:text-maroon transition-colors">
                {isPending && !courseToDelete ? <div className="h-5 w-5 border-2 border-maroon/30 border-t-maroon rounded-full animate-spin"></div> : (<><CheckIcon className="h-5 w-5" /></>) }
              </button>
            ) : (
      ...
      {courseToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 transition-opacity backdrop-blur-sm" onClick={() => !isPending && setCourseToDelete(null)}>
          <div className="bg-white rounded-2xl p-8 max-w-sm w-full shadow-2xl flex flex-col items-center transform transition-all" onClick={(e) => e.stopPropagation()}>
