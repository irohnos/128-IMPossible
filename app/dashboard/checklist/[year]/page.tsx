...
async function BatchChecklistContent({ params, searchParams }: { params: Promise<{ year: string }>, searchParams: Promise<{ query?: string; edit?: string; delete?: string; error?: string }> }) {
  ...
  const { query = "", edit, delete: deleteParam, error: submitError } = await searchParams;
  ...
  async function updateStudent(formData: FormData) {
    ...
    const advisers = await populateAdvisers();
    const { data: tData } = await supabaseAction.from('term').select('*');
    
    let adviserIdToSave = null;
    let termIdToSave = null;
 
    if (advisers && submittedAdviserName) {
      const matchedAdviser = advisers.find(adv => adv.name === submittedAdviserName);
      if (matchedAdviser) adviserIdToSave = matchedAdviser.id;
    }

    if (tData && submittedTermName) {
      const matchedTerm = tData.find(term => `${term.semester} ${term.academic_year}`.trim() === submittedTermName);
  ...
  const startRange = parseInt(`${year}00000`);
  const endRange = parseInt(`${year}99999`);
          ...
          <div className="relative z-10 bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form key={`form-${studentToEdit.student_number}`} action={updateStudent} className="flex flex-col flex-1 overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-5">
                {submitError && <div className="p-3 bg-red-50 text-red text-sm rounded-lg border border-red-100 break-words">{submitError}</div>}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  <div className="sm:col-span-1 text-left">
                    ...
                    <RestrictedInput restrictionType="name" type="text" name="student_fname" defaultValue={studentToEdit.student_fname || ""} placeholder="John" className="w-full bg-gray-50 text-gray-800 text-sm px-4 py-2.5 rounded-lg border border-gray-200 focus:border-maroon focus:bg-white outline-none transition-all" required />
                  </div>
                  <div className="sm:col-span-1 text-left">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="text-left">
                    ...
                    <input type="email" name="student_email" defaultValue={studentToEdit.student_email || ""} required placeholder="jbcruz@university.edu.ph" className="w-full bg-gray-50 text-gray-800 text-sm px-4 py-2.5 rounded-lg border border-gray-200 focus:border-maroon focus:bg-white outline-none transition-all" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-maroon-900 uppercase tracking-widest mb-1.5">Contact Number</label>
                    <RestrictedInput restrictionType="number" type="tel" placeholder="09XXXXXXXXX" className="w-full bg-gray-50 text-gray-800 text-sm px-4 py-2.5 rounded-lg border border-gray-200 focus:border-maroon focus:bg-white outline-none transition-all text-left"/>
                  </div>
                </div>
      ...
      {studentToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 transition-opacity backdrop-blur-sm">
          ...
          <div className="relative bg-white rounded-2xl p-8 max-w-sm w-full shadow-2xl flex flex-col items-center transform transition-all animate-in fade-in zoom-in-95 duration-200">
            ...
            {submitError && <p className="mt-3 w-full text-xs text-red font-medium bg-red-50 p-2 rounded border border-red-100 break-words text-center">{submitError}</p>}
